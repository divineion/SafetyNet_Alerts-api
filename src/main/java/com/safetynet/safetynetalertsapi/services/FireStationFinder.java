...
public class FireStationFinder {
	...
	/**
	 * This method retrieves all addresses covered by a given station.
	 * Steps : 
	 *  - it gets all fireStations,
	 *  - filters to retrieve the given station number,
	 *  - returns a collection of stations, each station can appear multiple times with different 
	 *  covered addresses. 
	 *  - Then based on the previous filter process, it the retrieves all addresses corresponding to the station number. 
	 * 
	 * @param stationNumber
	 * @return a list of addresses
	 */
	public List<String> getFireStationAddressesCoverage(int stationNumber) {
		List<FireStationDTO> fireStations = getAllFireStations()
				.stream()
				.filter(fireStationDTO -> fireStationDTO.getStation() == stationNumber)
				.collect(Collectors.toList());
		
		List<String> addresses = fireStations
				.stream()
				.map(f -> f.getAddress())
				.collect(Collectors.toList());
	...
	/**
	 * Retrieves a list of CoveredPersonDTO that represents covered persons by a station
	 * Steps : 
	 *  - calls {@link FireStationFinder#getFireStationAddressesCoverage()} method
	 *  - store addresses in a collection
	 *  - for each address, retrieves all persons living at this address
	 * 
	 * @param stationNumber
	...
	public List<CoveredPersonDTO> getCoveredPersons(int stationNumber) {
		...
		List<CoveredPersonDTO> coveredPersons = addresses.stream()
				...
				.map(mapper::fromPersonDTOToCoveredPersonDTO)
				.collect(Collectors.toList());
	...
	/**
	 * This method:
	 *  - calls {@link FireStationFinder#getFireStationCoverage()} to list the persons covered by a station,
	 *  - uses {@link PersonFilterService#filterChildren()} to retrieve children,
	 *  - uses count() to count children among them,
	 *  - uses {@link PersonFilterService#countAdults(List)} to count the adults among them. 
	 * 
	 * @param stationNumber
	 * @return {@link FireStationCoverageDTO} a class composed with a list of persons covered 
	 * 	by a station and a count of children and adults
	 */
	public FireStationCoverageDTO getFireStationCoverage(int stationNumber) {
		...
		return new FireStationCoverageDTO(
				coveredPersons, 
				filterService.countAdults(coveredPersons), 
